...
public class WebController {
    ...
    /**
     * 获取循环任务的执行历史（history.json）。
     */
    @Get
    @Mapping("/web/chat/loop/history")
    public Result loopHistory(@Param("sessionId") String sessionId, @Param("taskId") String taskId) {
        if (sessionId == null || sessionId.contains("..") || sessionId.contains("/") || sessionId.contains("\\")) {
            return Result.failure(400, "Invalid sessionId");
        }
        if (taskId == null || taskId.isEmpty()) {
        ...
        String workspace = engine.getWorkspace();
        try {
            java.nio.file.Path historyFile = org.noear.solon.codecli.command.builtin.LoopStateManager
                    .getStateDir(workspace, taskId).resolve("history.json");
            if (!java.nio.file.Files.exists(historyFile)) {
                return Result.succeed(new java.util.ArrayList<>());
            }
            String json = new String(java.nio.file.Files.readAllBytes(historyFile), java.nio.charset.StandardCharsets.UTF_8);
            ONode root = ONode.ofJson(json);
            java.util.List<Map> data = new java.util.ArrayList<>();
            if (root.isArray()) {
            ...
            return Result.failure(500, "Failed to load history: " + e.getMessage());
        }
    }

    /**
     * 获取循环任务的状态文件（NEXT.md / PROGRESS.md / DECISIONS.md）。
     */
    @Get
    @Mapping("/web/chat/loop/state")
    public Result loopState(@Param("sessionId") String sessionId, @Param("taskId") String taskId) {
        if (sessionId == null || sessionId.contains("..") || sessionId.contains("/") || sessionId.contains("\\")) {
            return Result.failure(400, "Invalid sessionId");
        }
        if (taskId == null || taskId.isEmpty()) {
        ...
        try {
            java.nio.file.Path stateDir = org.noear.solon.codecli.command.builtin.LoopStateManager
                    .getStateDir(workspace, taskId);
            String[] files = {"NEXT.md", "PROGRESS.md", "DECISIONS.md"};
            for (String f : files) {
                java.nio.file.Path fp = stateDir.resolve(f);
                if (java.nio.file.Files.exists(fp)) {
                    data.put(f, new String(java.nio.file.Files.readAllBytes(fp), java.nio.charset.StandardCharsets.UTF_8));
                }
            }
        } catch (Exception e) {
            // 返回已有内容
