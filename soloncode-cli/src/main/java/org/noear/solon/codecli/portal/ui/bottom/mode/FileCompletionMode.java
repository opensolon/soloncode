 ...
 * @ 文件补全面板。
 */
public class FileCompletionMode extends CompletionMode {
    public FileCompletionMode(CompletionState state, Supplier<String> promptSupplier) {
        super(state, promptSupplier);
    }

    @Override
    protected CompletionState.Mode mode() {
        return CompletionState.Mode.FILE;
    }

    @Override
    protected BottomListPanel.Item<Object> toItem(CompletionCandidate candidate) {
        BottomListPanel.Tone tone = candidate.isDirectory()
                ? BottomListPanel.Tone.SOFT
